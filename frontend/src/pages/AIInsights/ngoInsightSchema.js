...
You are an NGO intelligence analyst for DigitalSevaks — a platform that connects volunteers, donors, and NGOs.
 ...
 
{
  ...
  "DigitalSevaksSuggestions": [
    {
      "title": "Suggestion title",
      "suggestion": "Specific way DigitalSevaks could help this NGO",
...
- DigitalSevaksSuggestions must have 2 to 4 items
  ...
  impactInsights: [
    {
      ...
      insight:
        "Could not extract impact data from this NGO's public information.",
  ...
  DigitalSevaksSuggestions: [
    {
      ...
      suggestion:
        "Use DigitalSevaks to connect this NGO with skilled volunteers.",
...
export function normalizeNgoInsightResult(raw, websiteUrl) {
  if (!raw || typeof raw !== "object")
    return {
      ...DEFAULT_NGO_INSIGHT_RESULT,
      ngoSummary: {
        ...DEFAULT_NGO_INSIGHT_RESULT.ngoSummary,
        website: websiteUrl || "",
      },
    };
  ...
  const ngoSummary = {
    ...
    targetGroups:
      ensureStringArray(rawSummary.targetGroups).length > 0
        ? ensureStringArray(rawSummary.targetGroups)
        : def.ngoSummary.targetGroups,
    focusAreas:
      ensureStringArray(rawSummary.focusAreas).length > 0
        ? ensureStringArray(rawSummary.focusAreas)
        : def.ngoSummary.focusAreas,
  };

  // problemAreasDetected
  const problemAreasDetected = ensureArray(raw.problemAreasDetected).map(
    (item) => ({
      title: item.title || "Untitled Problem",
      description: item.description || "No description provided.",
      severity: normalizeSeverity(item.severity),
      evidenceFromWebsite:
        item.evidenceFromWebsite || "Not clearly available on website",
    }),
  );
  ...
  const impactInsights = ensureArray(raw.impactInsights).map((item) => ({
    ...
    metricOrEvidence:
      item.metricOrEvidence || "Not clearly available on website",
  }));

  // DigitalSevaksSuggestions
  const DigitalSevaksSuggestions = ensureArray(
    raw.DigitalSevaksSuggestions,
  ).map((item) => ({
  ...
  const aiScoreCards = SCORE_CARD_LABELS.map((label) => {
    const found = rawScoreCards.find(
      (c) =>
        c.label && c.label.toLowerCase().trim() === label.toLowerCase().trim(),
    );
    if (found) {
      return {
        label,
  ...
  return {
    ngoSummary,
    problemAreasDetected:
      problemAreasDetected.length > 0
        ? problemAreasDetected
        : def.problemAreasDetected,
    volunteerNeeds:
      volunteerNeeds.length > 0 ? volunteerNeeds : def.volunteerNeeds,
    impactInsights:
      impactInsights.length > 0 ? impactInsights : def.impactInsights,
    DigitalSevaksSuggestions:
      DigitalSevaksSuggestions.length > 0
        ? DigitalSevaksSuggestions
        : def.DigitalSevaksSuggestions,
    aiScoreCards,
  };
}
